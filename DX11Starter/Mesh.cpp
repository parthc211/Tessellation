...
Mesh::Mesh(const char * objFile, ID3D11Device * device)
{
	...
	std::vector<UINT> indices;			// Indices of theses verts
	...
	while (obj.good())
	{
		...
		{
			// Read the 3 numbers directly into a XMFLOAT3
			XMFLOAT3 norm;
			sscanf_s(
				chars,
				"vn %f %f %f",
		...
		{
			// Read the 2 numbers directly into a XMFLOAT2
			XMFLOAT2 uv;
			sscanf_s(
				chars,
				"vt %f %f",
		...
		{
			// Read the face indices into and array
			unsigned int i[12];
			...
			// 3D modelling packages use the bottom left as (0,0)
